...
# For Figure 1, we use regional summaries, all vessels, and cs_sensitivity <- FALSE
# We need port level metrics for the 4 combos of vessels and cs_sensitivity
...
all_combos <- rbind(all_combos, # for Figure 1
                    data.frame(data = all_combos$data[1],
                               scale = "region",
                               split_wc = all_combos$split_wc[1],
                               cs_sensitivity = FALSE,
                               include_all_vessels = TRUE))

for(i in 1:nrow(all_combos)) {

  data <- all_combos$data[i]
  scale <- all_combos$scale[i]
  split_wc <- all_combos$split_wc[i]
  cs_sensitivity <- all_combos$cs_sensitivity[i]
  include_all_vessels <- all_combos$include_all_vessels[i]
  ...
  #cs_sensitivity <- FALSE
  cs_str <- ifelse(cs_sensitivity, "cs_sensitivity", "")

  #Include all vessels, or just those that fish out of one port?
  #include_all_vessels <- TRUE
  vessel_str <- ifelse(include_all_vessels, "allVessels", "singlePortVessels")
...
if(cs_sensitivity==FALSE & include_all_vessels==TRUE & scale=="port") saveRDS(port_df, paste0("data/",data,"_",scale,"_ports",".rds"))
